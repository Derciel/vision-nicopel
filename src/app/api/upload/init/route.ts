 ...
 * Retorna: { uploadUrl, fileId, finalName }
 */
export async function POST(req: Request) {
  try {
    ...
    if (!fileName || !mimeType || !fileSize) {
      return NextResponse.json({ error: 'fileName, mimeType e fileSize são obrigatórios.' }, { status: 400 });
    }

    getDriveClient(); // inicializa authClient
    ...
    console.error('Erro em /upload/init:', e);
